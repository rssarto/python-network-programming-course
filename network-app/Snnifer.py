...
# This will suppress all messages that have a lower lever of seriousness than error messages, while running or loading Scapy
...
print("\n! Make sure to ruan this program as ROOT!\n")

# Asking the user for some parameters: interface on which to sniff, the number of packets fo sniff, time interval to sniff, the protocol
...
interface controller (MIC) or wireless network interface controller (WNIC) that causes the controller to
pass all traffic it receives to the central processing unit (CPU) rather than passing only the frames
that the controller is intended to receive.
This mode is normally used for packet sniffing that takes place on a router or on a computer connected
to a hub.
'''
try:
...
def packet_log(packet):
    ...
    if proto_sniff == "0":
        ...
        # Writing the data to the log file
        print("Time " + str(now) + " Protocol: " + proto_sniff.upper() + " SMAC: " + packet[0].src + " DMAC: " + packet[
